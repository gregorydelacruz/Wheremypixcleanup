...
// This is a server-side API endpoint for Mailchimp subscription
// It should be deployed to Vercel or another serverless platform
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    try {
      ...
      // Even if parsing fails, we want to return a valid JSON response
      responseData = { status: 'success', message: 'Subscription successful' };
    }

    if (!subscribeResponse.ok) {
      console.error('Mailchimp API error:', responseData);
      return res.status(subscribeResponse.status).json({ 
        message: responseData.title || 'Error subscribing to newsletter',
